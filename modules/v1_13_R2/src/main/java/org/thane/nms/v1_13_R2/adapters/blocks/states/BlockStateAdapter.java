    ...
    public BlockState read(JsonReader in) throws IOException {
        ...
        while (in.hasNext()) {
            if (in.peek() == JsonToken.NAME) {
                state = readState(gson, in, in.nextName(), state);
//                switch (in.nextName()) {
//                    case "material":
...
//                }
    ...
    public static void writeState(Gson gson, JsonWriter out, BlockState value) throws IOException {
        out.name("material");
        ...
        if (value.getData().getData() != 0) {
            out.name("material data");
            gson.getAdapter(MaterialData.class).write(out, value.getData());
        }
        if (value instanceof InventoryHolder) {
            if (((InventoryHolder) value).getInventory().getContents().length != 0
                    && Arrays.stream(((InventoryHolder) value).getInventory().getContents()).anyMatch(Objects::nonNull)) {
                out.name("inventory");
    ...
    public static BlockState readState(Gson gson, JsonReader in, String name, BlockState state) throws IOException {
        switch (name) {
            case "material":
                state = ((BlockStateMeta) new ItemStack(gson.getAdapter(Material.class).read(in)).getItemMeta()).getBlockState();
                break;
